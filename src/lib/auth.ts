...
export const USERS_KEY = 'speakout-a2.users.v1'
const SESSION_KEY = 'speakout-a2.session.v1'
...
export function storageKey(base: string): string {
  ...
  return user ? `speakout-a2.u.${user}.${base}` : base
}
...
export async function register(username: string, password: string, age?: number): Promise<AuthResult> {
  ...
  const salt = `sb2:${name.toLowerCase()}`
